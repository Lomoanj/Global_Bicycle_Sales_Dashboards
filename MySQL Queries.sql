...
select * from sales1 union all select * from sales;
select concat(c.firstname," ",c.middlename," ",c.lastname) as fullname,p.unitprice,concat(round(sum(s.salesamount)/1000,2),"K")  as Total_sales from (product as p join (select * from sales1 union select * from sales) as s on s.productkey = p.productkey)
join customer1 as c on c.customerkey = s.customerkey group by fullname order by total_sales desc limit 10;
...
select productkey,unitprice,count(orderquantity),discountamount,round(sum(unitprice*orderquantity * (1 - discountamount)),2) AS sales_amount from sales1 union all 
select productkey,unitprice,sum(orderquantity),discountamount,unitprice*orderquantity * (1 - discountamount)  from sales group by 1 order by sales_amount desc limit 10;

---- Method 2 -------

select *,round(sum(unitprice),2) as unitprice,sum(orderquantity) as no_of_quanties ,sum(discountamount) as dis,
...
from (select * from sales1 union all select * from sales) as k group by productkey order by sales_amount desc limit 10;
 ...
 select *,productstandardcost,sum(orderquantity) as total_quantity,round(sum(productstandardcost*orderquantity),2) as Productioncost from 
...
with pro as (select productkey,Salesamount,totalproductcost,taxamt,freight, sum(salesamount-(totalproductcost+taxamt+freight)) as Profit 
from (select * from sales1 union all select * from sales)  k group by productkey order by profit desc)
select sum(profit) from pro;
...
where year(date1) = 2012 group by monthfullname order by total_sales desc;
